defmodule PredictionsLandscape do
  ...
  def render(assigns) do
    ~H"""
    <div class="buses">
      <PredictionsRealtimeLandscape :if={{ length(@stopPredictionsRealtimeSet) > 0 and is_nil(@suppressed_messages.global_message) }} stopPredictionsSet={{@stopPredictionsRealtimeSet}} activeIndex={{@realtimeActiveIndex}} suppressed_messages={{@suppressed_messages}} twoColumn={{@twoColumn}} is_bus_interchange={{@is_bus_interchange}}/>
      <PredictionsScheduledLandscape :if={{ length(@stopPredictionsRealtimeSet) <= 0 and is_nil(@suppressed_messages.global_message) }} stopPredictionsSet={{@stopPredictionsScheduledSet}} activeIndex={{@scheduledActiveIndex}} suppressed_messages={{@suppressed_messages}} twoColumn={{@twoColumn}} is_bus_interchange={{@is_bus_interchange}}/>
