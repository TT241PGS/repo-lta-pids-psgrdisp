defmodule LandscapeTwoPaneBLayout do
  ...
  def render(assigns) do
    ...
    <div>
      ...
      <QuickestWayToLandscape services={{@prop.quickest_way_to}} :if={{get_in(@prop.current_layout_panes, ["pane2", "type", "value"]) == "quickest_way_to" and length(@prop.quickest_way_to) > 0}} />
      <AdvisoriesLandscape
        message={{@prop.message}}
        :if={{
          get_in(@prop.current_layout_panes, ["pane2", "type", "value"]) == "scheduled_and_ad_hoc_messages" and
          @prop.message != %{}
        }}
        current_pane="pane2"
        panes={{@prop.current_layout_panes}}
      />
      <IncomingBusLandscape incoming_buses={{@prop.incoming_buses}} :if={{get_in(@prop.current_layout_panes, ["pane2", "type", "value"]) == "next_buses_arriving_at_stop" and length(@prop.incoming_buses) > 0}} />
    </div>
    """
  end
end
